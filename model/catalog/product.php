...
class Product extends \Opencart\System\Engine\Model {
	...
	public function editProduct(int $product_id, array $data): void {
		$this->db->query("UPDATE `" . DB_PREFIX . "consignee` SET `model` = '" . $this->db->escape((string)$data['model']) . "', `sku` = '" . $this->db->escape((string)$data['sku']) . "', `upc` = '" . $this->db->escape((string)$data['upc']) . "', `ean` = '" . $this->db->escape((string)$data['ean']) . "', `jan` = '" . $this->db->escape((string)$data['jan']) . "', `isbn` = '" . $this->db->escape((string)$data['isbn']) . "', `mpn` = '" . $this->db->escape((string)$data['mpn']) . "', `location` = '" . $this->db->escape((string)$data['location']) . "', `variant` = '" . $this->db->escape(!empty($data['variant']) ? json_encode($data['variant']) : '') . "', `override` = '" . $this->db->escape(!empty($data['override']) ? json_encode($data['override']) : '') . "', `quantity` = '" . (int)$data['quantity'] . "', `minimum` = '" . (int)$data['minimum'] . "', `subtract` = '" . (isset($data['subtract']) ? (bool)$data['subtract'] : 0) . "', `stock_status_id` = '" . (int)$data['stock_status_id'] . "', `date_available` = '" . $this->db->escape((string)$data['date_available']) . "', `manufacturer_id` = '" . (int)$data['manufacturer_id'] . "', `shipping` = '" . (isset($data['shipping']) ? (bool)$data['shipping'] : 0) . "', `price` = '" . (float)$data['price'] . "', `points` = '" . (int)$data['points'] . "', `weight` = '" . (float)$data['weight'] . "', `weight_class_id` = '" . (int)$data['weight_class_id'] . "', `length` = '" . (float)$data['length'] . "', `width` = '" . (float)$data['width'] . "', `height` = '" . (float)$data['height'] . "', `length_class_id` = '" . (int)$data['length_class_id'] . "', `status` = '" . (bool)(isset($data['status']) ? $data['status'] : 0) . "', `tax_class_id` = '" . (int)$data['tax_class_id'] . "', `sort_order` = '" . (int)$data['sort_order'] . "', `date_modified` = NOW() WHERE `product_id` = '" . (int)$product_id . "'");
	...
	public function deleteProduct(int $product_id): void {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "consignee` WHERE `product_id` = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "product_attribute` WHERE `product_id` = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "product_description` WHERE `product_id` = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "product_discount` WHERE `product_id` = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "product_filter` WHERE `product_id` = '" . (int)$product_id . "'");
		...
		$this->db->query("UPDATE `" . DB_PREFIX . "consignee` SET `master_id` = '0' WHERE `master_id` = '" . (int)$product_id . "'");
	...
	public function editRating(int $product_id, int $rating): void {
		$this->db->query("UPDATE `" . DB_PREFIX . "consignee` SET `rating` = '" . (int)$rating . "', `date_modified` = NOW() WHERE `product_id` = '" . (int)$product_id . "'");
	}
	...
	// 	$query = $this->db->query("SELECT DISTINCT * FROM `" . DB_PREFIX . "consignee` p LEFT JOIN `" . DB_PREFIX . "product_description` pd ON (p.`product_id` = pd.`product_id`) WHERE p.`product_id` = '" . (int)$product_id . "' AND pd.`language_id` = '" . (int)$this->config->get('config_language_id') . "'");

	// 	return $query->row;
	// }
	public function getProduct(int $product_id): array {
		$query = $this->db->query("SELECT DISTINCT * FROM `
			" . DB_PREFIX . "consignee` c LEFT JOIN `
				" . DB_PREFIX . "consignee_description` 
				cd 
					ON (c.`consignee_id` = cd.`consignee_id`) 
					WHERE 
						c.`consignee_id` = '" . (int)$product_id . "' 
						AND 
	...
	// 	//$sql = "SELECT * FROM `" . DB_PREFIX . "consignee` p LEFT JOIN `" . DB_PREFIX . "product_description` pd ON (p.`product_id` = pd.`product_id`) WHERE pd.`language_id` = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getProducts(array $data = []): array {
		$sql = "
			SELECT 
				...
				cd.*
			FROM 
				`" . DB_PREFIX . "consignee` c 
			LEFT JOIN 
				`" . DB_PREFIX . "consignee_description` cd ON (c.`consignee_id` = cd.`consignee_id`)
			WHERE 
				cd.`language_id` = '" . (int)$this->config->get('config_language_id') . "'
		";
	
		if (!empty($data['filter_master_id'])) {
			$sql .= " AND c.`master_id` = '" . (int)$data['filter_master_id'] . "'";
		}
		...
		if (!empty($data['filter_location'])) {
			$sql .= " AND c.`location` LIKE '" . $this->db->escape((string)$data['filter_location'] . '%') . "'";
		}
	...
	public function getDescriptions(int $product_id): array {
		...
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "consignee_description` WHERE `consignee_id` = '" . (int)$product_id . "'");
	...
	public function getImages(int $product_id): array {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "consignee_image` WHERE `consignee_id` = '" . (int)$product_id . "' ORDER BY `sort_order` ASC");
	...
	// 	$sql = "SELECT COUNT(DISTINCT p.`product_id`) AS `total` FROM `" . DB_PREFIX . "consignee` p LEFT JOIN `" . DB_PREFIX . "product_description` pd ON (p.`product_id` = pd.`product_id`) WHERE pd.`language_id` = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getTotalProducts(array $data = []): int {
		$sql = "
			SELECT 
				COUNT(DISTINCT c.`consignee_id`) AS `total`
			FROM 
				`" . DB_PREFIX . "consignee` c 
			LEFT JOIN 
				`" . DB_PREFIX . "consignee_description` cd ON (c.`consignee_id` = cd.`consignee_id`)
	...
	public function getTotalProductsByTaxClassId(int $tax_class_id): int {
		$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "consignee` WHERE `tax_class_id` = '" . (int)$tax_class_id . "'");
	...
	public function getTotalProductsByStockStatusId(int $stock_status_id): int {
		$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "consignee` WHERE `stock_status_id` = '" . (int)$stock_status_id . "'");
	...
	public function getTotalProductsByWeightClassId(int $weight_class_id): int {
		$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "consignee` WHERE `weight_class_id` = '" . (int)$weight_class_id . "'");
	...
	public function getTotalProductsByLengthClassId(int $length_class_id): int {
		$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "consignee` WHERE `length_class_id` = '" . (int)$length_class_id . "'");
	...
	public function getTotalProductsByManufacturerId(int $manufacturer_id): int {
		$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "consignee` WHERE `manufacturer_id` = '" . (int)$manufacturer_id . "'");
